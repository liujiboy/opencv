...
void computeFeaturesAndMatches(Mat img1,Mat img2,vector<KeyPoint>&keypoints1,vector<KeyPoint>&keypoints2,vector<DMatch>&matches)
{
    //img1是图像1
    //img2是图像2
    //keypoints1是返回的图像1上的工件的特征点
    //keypoints2是返回的图像2上的工件的特征点
    //matches是返回的特征点的匹配
}
