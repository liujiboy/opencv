...
/*
 ...
 pnp：一个三维点的坐标数
 mnp：一个二维点的坐标数
 caminfilter：对相机位置参数进行处理的函数（输入）
 camoutfilter：对相机位置参数进行处理的函数（输出）
 filecnp：cnp+1？（如果cnp是3 rot+3 trans，那么filecnp是4 rot+3 trans）
 refcamsfname：优化后相机参数保存的文件
 refptsfname：优化后点保存的文件
 */
void sba_driver(char *camsfname, char *ptsfname, char *calibfname, int cnp, int pnp, int mnp,
                ...
                int filecnp, char *refcamsfname, char *refptsfname,int nconstframes)
{
...
int main(int argc, char *argv[])
{
    ...
    if(argc<6){
        fprintf(stderr, "使用方法 %s <相机位置> <点位置> <相机参数> <输出相机位置> <输出点位置> [<前几个相机固定>]\n", argv[0]);
        exit(1);
    }
    int nconstframes=0;
    if (argc==7) {
        nconstframes=atoi(argv[6]);
    }
    fprintf(stderr, "Starting BA with fixed intrinsic parameters\n");
    sba_driver(argv[1], argv[2], argv[3], cnp, pnp, mnp, quat2vec, vec2quat, cnp+1, argv[4],argv[5],nconstframes);
    return 0;
}
